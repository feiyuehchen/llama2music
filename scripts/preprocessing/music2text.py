...
def get_audio(audio_path, duration=10, target_sr=16000):
    ...
    try: 
        ...
        print(f'CANNOT GET AUDIO!! {audio_path}')
        audio = None
    return audio

def prepare_data(audio_dir, npy_dir, duration=10):
    print('================================================================================')
    print('PREPARE DATA...')
    wav_list = traverse_dir(audio_dir, is_sort=True, extension=('mp3', 'wav'))
    all_data = []
    count = 1
    for file_path in tqdm(wav_list):
        audio_tensor_list = get_audio(file_path, duration)
        if audio_tensor_list is None:
            continue
        number_of_chunks = range(audio_tensor_list.shape[0])
        for chunk, audio_tensor in zip(number_of_chunks, audio_tensor_list):
            time = f"{chunk * duration}:00-{(chunk + 1) * duration}:00"
        ...
        if len(all_data) > 1000:
            save_path = os.path.join(npy_dir, str(count)+'.npy')
...
def main_worker(args, duration=10, batch_size=20):
    print('================================================================================')
    print('LOAD DATASET...')
    ...
    model, _ = load_pretrained(args, lpmc_dir, model, mdp=config.multiprocessing_distributed)
    ...
    count = 0
    for subset in get_batch(wav_list, n=batch_size):
        ...
        for file_path in tqdm(subset):
            ...
            save_path = os.path.join(args.npy_dir, 'temp.npy')
            # print(f"save file: {save_path}")
            np.save(save_path, all_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--save_path", default='path/to/save/music2text_raw.json', 
        help="path for the output file",
    )
    parser.add_argument(
        "--lpmc_dir", default='path/to/lp-music-caps', 
        help="directory for the lpmc",
    )
    parser.add_argument(
        "--audio_dir", default='path/to/audio/dir', 
        help="directory for the input audio directory",
    )
    parser.add_argument(
        "--npy_dir", default='path/to/save/npy', 
        help="Directory for the output tensor audio data. This directory have to set if data is too large and you have to save the audio tensor.",
    )
    parser.add_argument(
        "--npy_id", default=0, 
    ...
    #     prepare_data(args.audio_dir, args.npy_dir)
    

    main_worker(args, args.duration)
