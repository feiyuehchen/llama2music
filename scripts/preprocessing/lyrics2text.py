...
# OpenAI creds
def getcreds(args):
    with open(args.cred_path, 'r') as file:
        # Load the YAML data
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--lyrics_dir", default='path/to/lyrics', 
    ...
    parser.add_argument(
        "--save_path", default='path/to/save/lyrics2text.json', 
        help="save path for the output lyrics and text",
    )
    parser.add_argument(
        "--clean_lyrics_save_path", default='path/to/save/clean_lyrics.json', 
    ...
    parser.add_argument(
        "--local_GPT", default=False, 
        help="using local GPT or not",
    )
...
""" 
    ...
    for subset in batch(lyrics_dataset, args.batch):
        ...
        print(f"{len(response_dataset)}/{len(lyrics_dataset)}")
        save_path = os.path.dirname(args.save_path) + f'/temp/lyrics2text_{len(response_dataset)}.json'
